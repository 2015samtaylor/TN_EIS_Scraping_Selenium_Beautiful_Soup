...
def stack_files(dir, str_match):   #needs to include string matching.
    ...
    for file in os.listdir(dir):
        if str_match in file:
            ...
            logging.warning('Issue with the str match in stack files')
...
school_dict_1 =  {'WDL' : ' School User (Wooddale Middle School - Achievement School District) ',
...
school_dict_2 =  {'WDL' : ' SCH (Wooddale Middle School - Achievement School District) ',
                    'KRB' : ' SCH (Kirby Middle School - Achievement School District) ',
                    'HIL' : ' SCH (Hillcrest High School - Achievement School District) ',
                    'BLF' : ' SCH_EC (Bluff City High School - Tennessee Public Charter School Commission) ', #old one is the charter school comission
...
def get_adm_audit_student_membership_loop(driver):
    try:
        logging.debug('Checking combined_dict_list definition')
        logging.debug(f'Type of combined_dict_list: {type(combined_dict)}')
        logging.debug(f'Contents of combined_dict_list: {combined_dict}')

        if not combined_dict:
            logging.debug('combined_dict_list is empty or not defined correctly')
        else:
            logging.debug('Calling get_adm_audit_student_membership')
            for (schools1, xpaths1), (schools2, xpaths2) in combined_dict:
                logging.info(f'Processing schools1: {schools1}, xpaths1: {xpaths1}')
                logging.info(f'Processing schools2: {schools2}, xpaths2: {xpaths2}')
